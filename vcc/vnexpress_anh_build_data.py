...
def make_photo_story_list():
    ...
    with open(DATA_DIR.joinpath('photo_story_url_list.json'), 'w', encoding='utf-8') as fp:
        json.dump({'photo_story_url_list': photo_story_url_list}, fp, indent=4, ensure_ascii=False)


@cli.command()
def build_data():
    with open(DATA_DIR.joinpath('photo_story_url_list.json'), 'r', encoding='utf-8') as fp:
    ...
    for url in tqdm(photo_story_url_list):
        try:
            ...
            for div_tag in article_tag.find_all('div', class_='item_slide_show clearfix'):
                description = []
